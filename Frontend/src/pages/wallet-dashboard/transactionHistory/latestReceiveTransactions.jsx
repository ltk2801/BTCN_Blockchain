...
const LatestReceiveTransactions = () => {
  const [currentPage, setCurrentPage] = useState(1);

  const [loading, setLoading] = useState(false);
  const [latestTransactionsReceive, setLatestTransactionsReceive] = useState(
    []
  );

  const walletFromStorage = JSON.parse(localStorage.getItem("currentWallet"));

  const getLatestTransactionsReceive = async () => {
    setLoading(true);
    try {
      const res = await Axios.get(
        `/api/transaction/getTransactionsReceive/${walletFromStorage.address}`
      );
      setLatestTransactionsReceive(res.data.transactions.reverse());
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const handleNextPage = () => {
    if (
      currentPage < Math.ceil(latestTransactionsReceive?.length / itemsPerPage)
    ) {
      setCurrentPage(currentPage + 1);
    }
  };
  ...
  const paginatedData =
    latestTransactionsReceive.length > 0
      ? latestTransactionsReceive.slice(
          ...
          {paginatedData.length === 0 ? (
            ...
            <>
              ...
              <div className="flex items-center justify-end p-4 text-slate-700 font-medium text-base  gap-10">
                <div>
                  Page {currentPage} of{" "}
                  {Math.ceil(latestTransactionsReceive?.length / itemsPerPage)}
                </div>
                <button
...
export default LatestReceiveTransactions;
